...
int set_rm32 (xed_decoded_inst_t* xedd, cpu_t* cpu, modrm_t modrm, xed_uint32_t value, mc_t* mc, int mc_max_cnt)
{
	...
	{
		printf("Write memory outside of current stack, address 0x%lx, value 0x%x\t", address, value);
		pmc->address = address;
		pmc->value = value;
	}
		...
		case 0x89:
			{
				...
				printf("sizeof(modrm_t) %ld, rm 0x%x, reg 0x%x, rm 0x%x\t", sizeof(modrm_t), modrm.mod, modrm.reg, modrm.rm);
